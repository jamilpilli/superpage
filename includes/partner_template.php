<?php
// Layout Base do Painel do Partner

function render_partner_header($title = "Partner Panel") {
    $user = get_logged_user();

    if (!in_array($user['role'], ['partner', 'admin'])) {
        http_response_code(403);
        die("Restricted access — Partners only.");
    }

    $uri = $_SERVER['REQUEST_URI'];
    $navItems = [
        ['href' => BASE_URL . '/partner',         'label' => 'Overview', 'icon' => 'dashboard',  'match' => fn($u) => rtrim($u, '/') === rtrim(BASE_URL . '/partner', '/')],
        ['href' => BASE_URL . '/partner/clients', 'label' => 'Clients',  'icon' => 'group',      'match' => fn($u) => strpos($u, '/partner/clients') !== false],
        ['href' => BASE_URL . '/partner/sites',   'label' => 'Sites',    'icon' => 'language',   'match' => fn($u) => strpos($u, '/partner/sites') !== false],
    ];
?>
<!DOCTYPE html>
...
<head>
    ...
    <title><?= htmlspecialchars($title) ?> – Partner <?= APP_NAME ?></title>
    ...
    <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-[#0d0d1a] border-r border-white/5 flex flex-col
        ...
        <div class="h-16 flex items-center px-6 border-b border-white/5 flex-shrink-0">
            <div class="flex items-center gap-2">
                <div class="w-7 h-7 rounded-lg bg-gradient-to-br from-[#914feb] to-[#685ef7] flex items-center justify-center">
                    <span class="material-symbols-outlined text-white" style="font-size:14px">handshake</span>
                </div>
                <span class="text-white font-black font-headline tracking-tight">PARTNER<span class="text-[#a9a4ff]">PANEL</span></span>
            </div>
        </div>
        ...
        <div class="p-4 border-t border-white/5">
            <div class="flex items-center gap-3 mb-3">
                <div class="w-8 h-8 rounded-full bg-gradient-to-br from-[#914feb] to-[#685ef7] flex items-center justify-center text-white text-xs font-black flex-shrink-0">
                    <?= strtoupper(substr($user['name'], 0, 1)) ?>
                </div>
                <div class="min-w-0">
                    <p class="text-sm font-bold text-white truncate"><?= htmlspecialchars($user['name']) ?></p>
                    <p class="text-xs text-[#a9a4ff] font-bold uppercase tracking-widest">Partner</p>
                </div>
            </div>
    ...
    <div class="md:ml-64 min-h-screen flex flex-col">
        ...
        <header class="h-16 border-b border-white/5 bg-[#0d0d1a]/80 backdrop-blur-xl flex items-center px-4 md:px-8 gap-4 sticky top-0 z-30">
            ...
            <div class="ml-auto flex items-center gap-3">
                <span class="hidden sm:flex items-center gap-1.5 px-3 py-1 bg-[#914feb]/20 text-[#c084fc] rounded-full text-xs font-black uppercase tracking-widest">
