...
<body class="text-gray-800 antialiased bg-white">
    ...
    $renderPhoneLink = function ($phone, $isWa, $classes) {
        ...
        $link = $isWa ? "https://wa.me/55{$onlyNumbers}" : "tel:{$onlyNumbers}";
        $icon = $isWa
    ...
    foreach ($blocks as $block) {
        ...
        switch ($type) {
            ...
            case 'footer':
                ...
                echo "<footer id='{$blockSlug}' class='text-white py-12 text-center mt-auto' style='background-color: var(--color-primary);'><div class='max-w-7xl mx-auto px-4'><h3 class='text-2xl font-bold mb-4 font-title'>{$blockTitle}</h3><p class='text-white opacity-80'>&copy; " . date('Y') . " Todos os direitos reservados.</p>{$footerPhoneHtml}</div></footer>";
                break;
            case 'gallery':
